...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!user) {
          return message.reply('vous devez mentionner la personne avec laquelle vous souhaitez être des frères.');
        }

        // si l user est un bot la commande ne fonctionne pas 
       
        if (user.bot) {
          return message.reply('vous ne pouvez pas être le frère d un bot.');
        }
        ...
        .setColor(color)
        .setDescription(`${message.author.username} souhaite être votre frère. Cliquez sur le bouton correspondant pour accepter ou refuser .`)
      ...
      const refuseButton = new Discord.MessageButton()
        ...
        user.send({embeds: [marryMess], components: [row]}).then(sentMessage => {
          ...
          button.reply({ content: `Vous avez accepté ${message.author.username} en tant que frère.`, ephemeral: true });

          author.send({content: `${user.username} a accepter que tu sois son frère : ${author}`})
            ...
            // Répondez à l'utilisateur avec un message confirmant le refus du lien
                button.reply({ content: `Vous avez refusé ${message.author.username} en tant que frère`, ephemeral: true });
          
                author.send({content: `${user.username} a refuser tu sois son frère : ${author}`})
                // Arrêtez le collecteur de boutons
                    collector.stop();
                }
            })
