...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        const vlog =  guild.channels.cache.get(db.get(`${message.guild.id}.logverif`))
     

          let rRole = db.get(`verole_${message.guild.id}`)

          const role = message.guild.roles.cache.get(rRole);
          const verifmenu = new Discord.MessageActionRow()
          .addComponents(
          ...
          let selectMenuOptions = [
            ...
            },
            {
                ...
                value: "embedsend", emoji: "✅"
            },
            {
                label: "Modifier un message par l'embed",
                value: "embededit", emoji: "💠"
            }
        ]
        ...
        message.channel.send({ content: `Config verif` }).then(async d => {
            let msgembed = await d.channel.send({ embeds: [embed], components: [embedBuilderActionRow] }).catch(async err => { return; })
                ...
                if (value === "embedsend") {
                    ...
                    message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ["time"] })
                        .then(async (collected) => {
                            ...
                            channel.send({ embeds: [embed], components: [verifmenu] })
