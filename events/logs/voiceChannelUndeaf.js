 ...
 const logschannel = member.guild.channels.cache.get(wass)
    ...
    if(logschannel) logschannel.send({
        embeds: [new Discord.MessageEmbed()
            .setColor(color)
           .setDescription(`**${executor.username}**${executor.discriminator} (\`${executor.id}\`) a donné la permissions d'écouter a **${member.user.username}**#${member.user.discriminator} (\`${member.id}\`) dans le salon [\`${channel.name}\`](https://discord.com/channels/${channel.guild.id}/${channel.id})`)
            .addField("Type de mute:", `${deafType}`)
            .setFooter(` Clarity ${client.config.version}` )
            .setAuthor("🔇 Demute casque d'un membre")
        ]
    }
    )
